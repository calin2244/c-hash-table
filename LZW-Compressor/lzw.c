...
void calculate_char_frequencies(HashTable* ht, const char* query) {
    ...
    for (size_t i = 0; i < strlen(query); ++i) {
        ...
        if (ht_has_key(ht, tempKey)) {
            ...
            ht_insert(ht, tempKey, &initial_fr, 1);  // Pass the correct key length
        }
...
CompressedArray lzw_encoding(HashTable* ht, const char* query){
    size_t pos = 0;
    ...
    for(int i = 0; i <= 255; ++i){
        ...
        tempKey[0] = key;
        tempKey[1] = '\0';
        ht_insert(ht, tempKey, &i, sizeof(int));
    }
    ...
    for(int i = 0; i < strlen(query); ++i){
        ...
        if(ht_has_key(ht, pc)){
           strcat(p, c);
            ...
            void* itm = ht_get_item(ht, p);
            if(itm)
                compressed_ar[pos++] = *(int*)itm;
            
            
            ht_insert(ht, pc, &code, sizeof(int));
            code++;
...
int main(int argc, char* argv[]){
    HashTable* hash_t = ht_create(10032, jenkins_hash_func, CHAINING);
    char query[256];
    if(argc > 1)
    ...
    CompressedArray compressed = lzw_encoding(hash_t, query);
